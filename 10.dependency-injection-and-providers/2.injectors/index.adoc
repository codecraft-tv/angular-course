...
We request the token `EmailService` from the `childInjector`, it can't find that token locally so it asks it's parent injector which returns the instance it had cached from a previous direct request.
